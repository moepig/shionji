...
- 転送設定の一覧 (状態ドット / 接続トグル / 検索済みエンドポイントの要約) + 詳細ペイン
...
- SSO トークン切れを検知し、詳細ペインの「SSO ログイン」ボタンから**アプリ内でブラウザ承認ログイン**できる (SDK のデバイス認可フロー。トークンは AWS CLI と `~/.aws/sso/cache` を共有するため CLI 側もログイン済みになる)。ログイン成功後は自動で再検索 / 再接続
...
- **リソース自動検索**: ローカルの HTTP スタブが AWS の JSON / Query 両プロトコルで応答し、実 SDK のシリアライズとアンマーシャルを通したまま、名前 glob の客側フィルタ、タグ条件の送出、ページング、エラー分類を確認する
...
| 自動検索の証跡 | 検索条件がどの実リソースに特定されたか (表示名 + ARN / インスタンス ID + エンドポイント) |
