...
ヘキサゴナル (ポートとアダプタ) 構成をとる。ドメイン層が外部との境界をインターフェースとして定義し、インフラ層がそれを実装する。GUI フレームワークへの依存は最上位の 1 プロジェクトに閉じ込める。

本ドキュメントは層の分け方、ポートと実装の対応、および層をまたぐ方針を扱う。ドメイン層の型構造は、[ドメインモデル](domain-model.md) を参照。

## プロジェクトと依存方向

プロジェクト間の参照関係を図示すると、次の図のようになる。矢印は参照する側から参照される側へ向く。
...
プレゼンテーション層が定義するポートは、すべて WinUI ヘッドが実装する。UI スレッドへの投げ込み、クリップボード、トースト通知、フォルダ選択、ウィンドウの表示、アプリ設定の読み書きとテーマの適用、登録済みコマンドの起動が該当する。

アプリ設定のうち自動起動だけは、設定ファイルではなく OS の自動起動の登録を状態とする。OS 側から解除された場合に食い違わないためである。

## 接続の流れ

接続要求を受けてから状態変更を返すまでのやり取りを図示すると、次の図のようになる。
...
ファイルへの整形は JSON Lines とする。詳細フィールドのキーが実装側の識別子と対応し、機械的な突き合わせに耐えるため。書式は、[設定](../usage/config.md#ログ) を参照。
