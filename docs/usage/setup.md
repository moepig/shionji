...
Shionji を初めて使うまでの手順を示す。設定項目そのものの説明は [設定](config.md) を参照。
...
Windows App SDK はアプリに同梱されているため、別途の導入は要らない。AWS CLI も接続には要らない (SSO ログインをコマンドで行う場合のみ使う)。
...
踏み台側にも準備が要る。EC2 を踏み台にする場合は SSM Agent が動作していて Systems Manager の管理対象になっていること、ECS を踏み台にする場合はタスクで ECS Exec が有効になっていることが必要。
